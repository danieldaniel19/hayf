...
struct ForteMotivationAnchorScreen: View {
    let selectedAnchors: Set<MotivationAnchor>
    @Binding var motivationNote: String
    let progressStep: Int
    let totalSteps: Int
    let onToggle: (MotivationAnchor) -> Void
    let onBack: () -> Void
    let onExit: () -> Void
    let onContinue: () -> Void
                ...
                ScrollView(showsIndicators: false) {
                    VStack(alignment: .leading, spacing: 0) {
                        ...
                        ForteStackedChoiceList(
                            items: choiceItems,
                            selectedIDs: Set(selectedAnchors.map(\.id))
                        ) { id in
                            guard let anchor = MotivationAnchor(rawValue: id) else { return }
                            onToggle(anchor)
                        }
    ...
    private var progressHeader: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            HStack(spacing: 12) {
                ...
                HStack(spacing: 8) {
                    ForteMotivationHeaderButton(systemName: "arrow.left", action: onBack)
                        .accessibilityLabel("Back")

                    ForteMotivationHeaderButton(systemName: "xmark", action: onExit)
    ...
    private var hero: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            Text("Consistency usually protects something.\nHelp me understand you better.")
    ...
    private var continueAction: some View {
        ...
        .buttonStyle(ForteMotivationPrimaryButtonStyle(isEnabled: !selectedAnchors.isEmpty))
        .disabled(selectedAnchors.isEmpty)
        .accessibilityHint(selectedAnchors.isEmpty ? "Select at least one motivation first." : "Shows the next onboarding step.")
        .padding(.horizontal, 24)
        .padding(.top, 8)
        .padding(.bottom, 18)
...
private struct ForteMotivationHeaderButton: View {
...
private struct ForteMotivationPrimaryButtonStyle: ButtonStyle {
    let isEnabled: Bool
