...
Status: target architecture for the LangGraph planning refactor that begins after Athlete Blueprint acceptance.
...
This document defines the target model and the refactor branch strategy. The older model remains the stable implementation until the LangGraph refactor proves it is better end to end.
...
1. document and schema contracts
2. durable artifacts for `training_architectures` and graph traces
3. LangGraph service spike
4. Supabase Edge Function bridge to the orchestration service
5. Training Architecture graph
6. specialist coach subgraphs and tools
7. Fitness Strategy generation from Training Architecture
8. two-week Plan generation from Training Architecture
9. real onboarding-case evaluation and rollback decision
...
The master coach orchestrator is the top-level graph node or subgraph. It:
...
Specialist coaches are bounded worker agents or subgraphs. They reason inside the orchestrator's frame:
...
Onboarding Intake output
  ...
  -> Training Architecture Graph
      -> Master Coach Framing
      -> Specialist Coach Subgraphs
      -> Master Coach Synthesis
...
- Specialist coaches can be tool-using agents, but only inside bounded subgraphs with typed outputs.
